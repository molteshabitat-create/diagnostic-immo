...
--- FIN DES DONNÉES DVF ---
Ces données DVF sont BEAUCOUP plus fiables que les prix demandés des annonces comparables car ce sont des ventes réellement conclues. RÈGLE CRITIQUE sur la façon de les utiliser : si la dispersion entre le prix/m² minimum et maximum est forte (ex : rapport de plus de 2x entre min et max, comme 1000€/m² à 4500€/m²), la MÉDIANE GLOBALE n'est PAS un point d'ancrage fiable — elle mélange probablement des biens très hétérogènes (bâti ancien/rural bon marché et biens récents de meilleure qualité dans le même secteur). Dans ce cas, NE TE CALE PAS sur la médiane : regarde individuellement la liste des transactions exemples fournie, identifie celle(s) dont le prix/m² est le plus élevé et qui semble(nt) correspondre à un bien de meilleure qualité/plus récent (prix/m² nettement au-dessus de la médiane) — c'est CETTE vente-là qui est le vrai comparable pour un bien récent et bien noté DPE comme celui diagnostiqué, pas la médiane qui inclut des biens anciens sans rapport. Explique ce raisonnement explicitement dans "estimation_prix" (ex : "la médiane du secteur est tirée vers le bas par du bâti ancien ; la vente à Xk€/Ym² (Z€/m²) est bien plus représentative d'un bien récent comme celui-ci"). Utilise cette transaction représentative comme point d'ancrage principal, pas la médiane brute. Si la dispersion est faible (min et max proches), la médiane reste un bon ancrage direct. Ensuite, applique l'ajustement DPE (+7% à +15%, sans te limiter par défaut au bas de cette fourchette si le bien a un vrai avantage DPE net face à son point d'ancrage retenu) et les autres facteurs qualitatifs. Le fichier DVF ne contient PAS l'année de construction des biens vendus : tu ne peux pas la vérifier directement, mais un prix/m² nettement supérieur à la médiane locale est un bon indice indirect de meilleure qualité/récence. Si le nombre de transactions trouvées est très faible (1 ou 2), précise-le explicitement ("échantillon très réduit, N vente(s) seulement") et reste prudent dans la formulation plutôt que de présenter ce chiffre comme une moyenne fiable — mais ne l'écarte pas pour autant, une vraie vente récente reste plus fiable qu'un prix demandé, même seule. Si la recherche a dû être élargie au-delà du code postal exact (rayon en km plutôt que "code postal exact"), précise-le brièvement dans "estimation_prix" (ex: "transactions élargies à X km, la commune précise ayant peu de ventes recensées") ET ajoute cette mise en garde si le bien est en zone frontalière (notamment proximité Luxembourg, Suisse, Allemagne — fréquent en Lorraine/Grand Est/Alsace) : la proximité immédiate d'une frontière augmente systématiquement les prix par rapport aux communes plus éloignées, à cause du pouvoir d'achat des travailleurs frontaliers. Si un écart existe entre le prix médian DVF et les prix demandés des comparables, signale-le explicitement.`
        });
      } else if (mode === 'prix') {
        content.push({
          type: 'text',
