...
Règles impératives :
- RÈGLE LA PLUS IMPORTANTE DE TOUT CE DOCUMENT, PRIME SUR TOUTES LES AUTRES : ce rapport n'est PAS destiné à décrire ou traduire un DPE — il est destiné à un agent/chasseur qui doit VENDRE le bien et NE JAMAIS être pris au dépourvu en rendez-vous. Une description technique neutre ("isolation probable moyenne", "chaudière fioul") NE SERT À RIEN à un agent : il la connaît déjà ou peut la lire sur le DPE lui-même. Ce qui lui manque, c'est SAVOIR QUOI DIRE. Donc pour "enveloppe_thermique" ET "chauffage_ventilation" (les deux champs les plus consultés), tu DOIS TOUJOURS terminer le texte par une phrase commençant par "💬 À dire si on vous pose la question :" suivie de la réponse orale toute prête, rassurante mais honnête, que l'agent peut réciter telle quelle.
  L'exemple ci-dessous (chaudière fioul) est UNIQUEMENT une illustration du NIVEAU de réponse attendu — ce n'est PAS une liste fermée de cas à reconnaître. Utilise TA PROPRE EXPERTISE technique et immobilière pour repérer, sur CHAQUE bien analysé, quel que soit le sujet (chauffage, isolation, toiture, électricité, façade, plomberie, ventilation, menuiseries, ou tout autre point que tu identifies comme sensible), ce qu'un acheteur pourrait objecter — et trouve toi-même l'angle honnête et rassurant qui répond à cette objection, exactement comme tu le ferais pour n'importe quel autre point que la chaudière fioul. Le raisonnement à reproduire, pas le cas précis : (1) identifier un fait qui pourrait inquiéter un acheteur, (2) trouver ce qui le relativise réellement (entretien, contexte de marché, coût maîtrisé, non-spécificité au bien, etc.), (3) formuler ça comme une phrase orale confiante, jamais défensive. Exemple exact du niveau attendu — pour une chaudière fioul (remplacement à l'identique interdit depuis 2022) : ne t'arrête PAS à "chaudière fioul, remplacement à l'identique interdit depuis 2022" — ajoute "💬 À dire si on vous pose la question : elle est entretenue et fonctionne bien ; de toute façon le marché évolue vers les pompes à chaleur pour tous les biens, gaz compris, ce n'est pas spécifique à celui-ci." Applique ce même réflexe à CHAQUE point technique substantiel que tu identifies, sans te limiter aux exemples cités : le fait seul ne suffit jamais, il faut toujours l'angle de vente/réponse qui va avec, trouvé par ton propre jugement d'expert. S'il n'y a vraiment rien à signaler sur un point (bien récent, rien à anticiper), dis-le simplement sans forcer un angle de vente artificiel.
...
{
  "verdict_global": "string - UNE seule phrase de synthèse (15-25 mots), à rédiger EN DERNIER après avoir déterminé tout le reste du rapport, orientée PRÉPARATION AGENT (pas description neutre du bien) : dit clairement si le bien se vend facilement ou s'il y a un point à anticiper avant le rendez-vous. Exemple : 'Bien vendable sans souci, 1 point à anticiper : la chaudière fioul — réponse prête ci-dessous.' Si aucun point notable : 'Bien sain, rien de particulier à anticiper en visite.'",
  "enveloppe_thermique": "string ou null (null uniquement en mode estimation de prix) - constat bref sur l'isolation probable (murs, toiture, vitrage), PUIS OBLIGATOIREMENT terminé par '💬 À dire si on vous pose la question :' suivi de la réponse orale toute prête pour l'agent (voir règle impérative en tête de ce prompt — ce n'est pas optionnel).",
  "chauffage_ventilation": "string ou null (null uniquement en mode estimation de prix) - constat bref sur le système de chauffage/ventilation visible ou déclaré, PUIS OBLIGATOIREMENT terminé par '💬 À dire si on vous pose la question :' suivi de la réponse orale toute prête pour l'agent (voir règle impérative en tête de ce prompt — ce n'est pas optionnel).",
  ...
  "questions_reponses": "array d'objets {question, reponse} ou tableau vide - EN MODE DIAGNOSTIC/PRIX : 5 objets maximum. EN MODE QUESTIONS & PIÈGES : 8 à 10 objets (voir instruction dédiée plus bas). Anticipe les questions qu'un acheteur poserait probablement en visite en voyant ce bien, avec une réponse professionnelle prête à l'oral pour l'agent/chasseur. Vise le maximum autorisé pour ce mode dès que le bien a suffisamment de points distincts pour les justifier — couvre une bonne diversité de sujets (isolation, chauffage, budget, annonce, équipements spécifiques) plutôt que plusieurs questions proches sur le même sujet. Chaque réponse doit être équilibrée : reconnaître le point factuellement (jamais nier ou minimiser à l'excès), donner un ordre de grandeur chiffré si pertinent, et rassurer sur ce qui est déjà positif si applicable. Base-toi uniquement sur les points déjà identifiés ailleurs dans le rapport (points_vigilance, enveloppe_thermique, chauffage_ventilation) en mode diagnostic, ou sur les photos/DPE/annonce en mode pièges — ne réinvente rien de nouveau. Exemple de format : {\\'question\\': \\'Pourquoi je ne vois pas de VMC ?\\', \\'reponse\\': \\'Elle n'est pas visible sur ces photos, ce sera à vérifier ensemble en visite — si absente, l'installation coûte entre 1 500€ et 4 000€, déjà anticipé dans notre estimation.\\'}",
  "pieges_techniques": "array de strings ou null (null sauf en mode Questions & Pièges ou mode Extérieurs) - voir instruction dédiée plus bas pour ces modes.",
  "pieges_administratifs": "array de strings ou null (null sauf en mode Questions & Pièges ou mode Extérieurs) - voir instruction dédiée plus bas pour ces modes.",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (validComparables.length > 0) {
      content.push({
        type: 'text',
        text: "--- DÉBUT DES ANNONCES COMPARABLES : ce sont D'AUTRES biens du même secteur, PAS le bien diagnostiqué ci-dessus. Ne les mélange jamais avec les photos/infos du bien analysé. Chaque bien comparable ci-dessous regroupe sa/ses propre(s) photo(s) ET sa propre description texte — ne mélange pas les infos d'un bien comparable avec un autre. Pour le prix et la surface de chaque comparable : vérifie d'abord la photo, et si ce n'est pas clairement lisible dessus, vérifie la description texte qui l'accompagne (l'utilisateur y indique parfois le prix manuellement si la capture ne le montre pas bien). Si ni la photo ni le texte ne donnent un prix ou une surface exploitable pour un comparable donné, ne l'invente pas — ignore ce comparable précis dans le calcul plutôt que de deviner un chiffre. RÈGLE IMPORTANTE SUR LES ÉQUIPEMENTS NON MENTIONNÉS (garage, climatisation, piscine, etc.) : pour un COMPARABLE, l'absence de mention d'un équipement dans le texte/la photo ne veut PAS dire qu'il est absent — les annonces courtes ou les captures partielles omettent souvent des équipements qui existent bel et bien. NE TRAITE JAMAIS le silence d'un comparable sur un équipement comme une confirmation d'absence (SEULE EXCEPTION : l'ascenseur pour un appartement, où le silence est un signal fiable — voir règle dédiée). Pour tout autre équipement, si un comparable ne mentionne rien, traite ce point comme INCONNU pour ce comparable précis, pas comme 'absent' — ne t'en sers pas comme argument de comparaison sur ce critère précis avec le bien diagnostiqué. Cette règle est différente pour le BIEN DIAGNOSTIQUÉ lui-même, où les cases cochées, les caractéristiques complémentaires et l'analyse photo détaillée donnent des informations bien plus fiables. Utilise l'ensemble uniquement pour la comparaison de prix demandée dans 'estimation_prix'. ---"
      });
...
- Brise-vue installé : ${form?.brise_vue ? 'oui' : 'non déclaré'}
- Terrain arboré : ${form?.terrain_arbore ? 'oui' : 'non déclaré'}
- Abri bois/vélos (auvent, appentis) : ${form?.abri_bois_velo ? 'oui' : 'non déclaré'}
- Abri de jardin : ${form?.abri_jardin || 'non renseigné'}${form?.abri_jardin && form?.abri_jardin_surface ? ` (environ ${form.abri_jardin_surface}m²)` : ''}
...
${mode === 'prix' ? "\n⚠️ MODE ESTIMATION DE PRIX UNIQUEMENT : l'utilisateur ne veut PAS un diagnostic technique complet, seulement un positionnement de prix basé sur les annonces comparables. Concentre l'essentiel de ton effort sur 'estimation_prix' et 'verdict_global' (qui doit résumer uniquement le positionnement de prix, pas l'état technique du bien). Pour tous les autres champs techniques (enveloppe_thermique, chauffage_ventilation, points_vigilance, budget_estime, budget_detail, budget_postes, cout_fonctionnement_annuel, arguments_negociation, questions_reponses, score_transparence, pieges_techniques, pieges_administratifs), renvoie null ou un tableau vide selon le type attendu — ne tente pas de les remplir, même partiellement, ce n'est pas ce qui est demandé dans ce mode.\n" : ''}${mode === 'pieges' ? `
⚠️ MODE QUESTIONS & PIÈGES UNIQUEMENT : l'utilisateur ne veut NI diagnostic technique complet NI estimation de prix — il veut se préparer à un rendez-vous ou une visite avec de quoi répondre du tac au tac. Renvoie null ou un tableau vide pour TOUS les champs techniques habituels (enveloppe_thermique, chauffage_ventilation, points_vigilance, budget_estime, budget_detail, budget_postes, cout_fonctionnement_annuel, estimation_prix, arguments_negociation, score_transparence, analyse_annonce, fiabilite_annonce, fiabilite_annonce_detail) — ne tente pas de les remplir. Concentre TOUT ton effort sur ces 4 champs uniquement :
- "verdict_global" : 1 phrase (25 mots max) résumant le niveau de préparation nécessaire (ex : "Bien globalement sain, 2 points sensibles à anticiper : la toiture et l'ancienneté du chauffage.")
- "questions_reponses" : 8 à 10 paires question/réponse (au lieu de 4-5 habituellement) — des VRAIES questions qu'un acheteur méfiant ou bien informé pourrait poser en visite, avec des réponses courtes, honnêtes et convaincantes prêtes à l'oral (20-30 mots par réponse). Mélange questions techniques (chauffage, isolation, travaux), questions sur l'historique (pourquoi ils vendent, depuis quand en vente), et questions piège classiques (négociation, défauts cachés, charges).
- "pieges_techniques" (nouveau champ, tableau de strings) : 5 à 8 points techniques précis à vérifier ou à anticiper AVANT la visite ou la signature — des choses qu'un acheteur avisé ou un diagnostiqueur pourrait pointer et qu'il vaut mieux avoir déjà en tête (ex : "Vérifier la date du dernier ramonage — souvent demandé par l'acheteur", "Chaudière de plus de 15 ans : prévoir une réponse sur sa durée de vie restante"). Base-toi sur les photos, le DPE et l'annonce fournis. RESTE PROPORTIONNÉ sur l'entretien de chaque équipement — ne traite pas tous les systèmes de la même façon par réflexe : le ramonage d'un poêle/insert est une VRAIE obligation légale annuelle (assurance peut refuser un sinistre sans preuve), une chaudière gaz a un entretien annuel réglementaire aussi — ce sont des points réels à lister. Une PAC air-air, en comparaison, n'a QUASIMENT AUCUN entretien contraignant pour un acheteur (nettoyage de filtre occasionnel par le propriétaire, contrôle professionnel seulement requis au-delà de 2kg de fluide frigorigène, tous les 2 ans) — ne la mets PAS sur le même plan qu'un poêle ou une chaudière comme "point à vérifier", ce serait exagérer un non-sujet et donner une fausse impression de complexité à l'acheteur. N'ALIGNE PAS systématiquement tous les équipements sur le même niveau d'inquiétude — hiérarchise selon la réalité de la contrainte. À L'INVERSE, si un matériau ou équipement de qualité supérieure est mentionné (dans les caractéristiques complémentaires, l'annonce, ou visible sur les photos) — par exemple un enduit de façade "collé filet" (plus cher, plus durable, limite les fissures, contrairement à un enduit traditionnel basique) — transforme-le en ARGUMENT DE VENTE positif à connaître pour l'agent, pas seulement en point de vigilance : précise que ce choix de matériau justifie en partie le niveau de prix et la durabilité, une info à valoriser en visite plutôt qu'à simplement surveiller pour des fissures. NE CONFONDS JAMAIS isolation/carrelage d'un garage avec habitabilité : un garage isolé et/ou carrelé mais SANS système de chauffage propre reste un garage — c'est un vrai plus modeste (protection contre le gel pour le véhicule/le matériel stocké, confort correct pour du bricolage), mais ce n'est PAS un "espace de vie" ni une pièce potentiellement habitable, et tu ne dois jamais le présenter comme tel sauf si un chauffage dédié est explicitement mentionné. Reste factuel sur ce que l'isolation apporte réellement (hors gel, pas d'humidité excessive) sans suggérer une polyvalence d'usage qui n'existe pas.
- "pieges_administratifs" (nouveau champ, tableau de strings) : 4 à 6 points juridiques/administratifs classiques à ne pas rater (ex : "Vérifier que le DPE n'est pas périmé (validité 10 ans, ou moins pour les DPE établis avant juillet 2021)", "Si copropriété : avoir le carnet d'entretien et les 3 derniers PV d'AG prêts", "Vérifier l'absence de servitude non mentionnée"). Reste général et factuel, pas de conseil juridique engageant.
Sois concret et direct, pense "ce qu'un agent aguerri sait déjà mais qu'un débutant oublierait", pas des généralités vagues.` : ''}${mode === 'exterieurs' ? `
⚠️ MODE EXTÉRIEURS UNIQUEMENT : l'utilisateur ne veut NI diagnostic technique intérieur NI estimation de prix — il veut se préparer spécifiquement au tour du jardin/terrain/extérieurs en visite. Renvoie null ou un tableau vide pour TOUS les champs habituels (enveloppe_thermique, chauffage_ventilation, points_vigilance, budget_estime, budget_detail, budget_postes, cout_fonctionnement_annuel, estimation_prix, arguments_negociation, score_transparence, analyse_annonce, fiabilite_annonce, fiabilite_annonce_detail) — ne tente pas de les remplir. Concentre TOUT ton effort sur ces 4 champs, exclusivement centrés sur l'EXTÉRIEUR du bien (jardin, terrain, clôtures, portail, allée, terrasse, façade vue de l'extérieur, toiture vue de l'extérieur, dépendances, piscine, arbres, drainage/évacuation des eaux de pluie) — AUCUNE donnée DPE ni surface de terrain précise n'est nécessaire pour ce mode, base-toi uniquement sur les photos et le texte de l'annonce/caractéristiques complémentaires fournis :
- "verdict_global" : 1 phrase (25 mots max) résumant l'état général des extérieurs et le point principal à anticiper si applicable (ex : "Jardin bien entretenu, un point à anticiper : la clôture côté nord visiblement ancienne.")
- "questions_reponses" : 6 à 8 paires question/réponse — des VRAIES questions qu'un acheteur poserait en visitant le jardin/l'extérieur (entretien, vis-à-vis, exposition, état de la façade/toiture visible de dehors, clôtures, portail, drainage, arbres proches de la maison), avec des réponses courtes et honnêtes prêtes à l'oral (20-30 mots par réponse).
- "pieges_techniques" : 4 à 6 points précis à vérifier ou valoriser AVANT la visite concernant spécifiquement l'extérieur (ex : "Vérifier l'état de la clôture côté nord, visiblement datée sur les photos", "Enduit de façade en bon état apparent, argument de qualité à mentionner"). Applique la même logique de proportionnalité que pour l'intérieur : ne traite pas un simple entretien courant du jardin comme un point d'alerte, et transforme un matériau/aménagement de qualité en argument de vente plutôt qu'en simple remarque neutre.
- "pieges_administratifs" : 3 à 5 points juridiques/administratifs liés spécifiquement au terrain/extérieur (ex : "Vérifier l'absence de servitude de passage ou de vue non mentionnée dans l'annonce", "Confirmer les limites de propriété avec le bornage si terrain non clôturé sur tout son périmètre", "Vérifier le règlement de lotissement si applicable (charges communes, contraintes d'aspect)").
ÉVALUATION VISUELLE DE L'ÉTAT (terrasse, palissade/clôture) : contrairement aux faits déclarés (piscine, brise-vue, terrain arboré, abri de jardin — traite-les comme certains, pas à re-vérifier), l'ÉTAT de la terrasse et des palissades/clôtures doit être évalué depuis les PHOTOS fournies, comme pour la cuisine/salle de bain en mode diagnostic : repère si le matériau/l'aspect semble récent (dalles/lames propres, teinte uniforme, palissade droite et sans usure) ou ancien/dégradé (fissures, mousse, décoloration, palissade voilée ou grisée). Si récent/soigné → argument de vente positif à mentionner. Si ancien/dégradé → point à anticiper, avec un ordre de grandeur de coût si tu peux l'estimer (terrasse à refaire : compte 60€-150€/m² selon matériau, déjà sourcé ailleurs dans ce prompt ; clôture/palissade à remplacer : quelques centaines d'euros par mètre linéaire selon matériau). Si l'état n'est pas clairement visible sur les photos fournies, dis-le simplement plutôt que de deviner.
TERRASSE SUR PLOT (plots réglables) : si ce type de terrasse est mentionné (annonce ou caractéristiques complémentaires), ajoute systématiquement une question ou un point technique sur ce qu'il y a EN DESSOUS — une terrasse sur plot peut reposer sur une vraie dalle béton (fondation solide et durable, stable dans le temps) ou directement sur de la terre compactée/un remblai (moins stable, risque de tassement irrégulier des plots avec le temps, drainage moins maîtrisé). C'est une information invisible sur les photos mais importante à faire préciser par le vendeur avant la visite ou pendant celle-ci — formule ça comme une vraie question à poser, pas une simple remarque.
ASPECT GÉNÉRAL DE LA MAISON VUE DE DEHORS : ne te limite pas au jardin/terrain séparément de la maison — évalue aussi l'impression d'ensemble que donne la maison elle-même vue de l'extérieur (premier regard en arrivant devant le bien) : cohérence du style architectural, propreté et état de la façade/des menuiseries extérieures, harmonie des couleurs/matériaux, état visible de la toiture depuis le sol, aspect général soigné ou au contraire négligé/vieillot. C'est souvent la toute première impression qui influence un acheteur avant même d'entrer — un point à valoriser explicitement s'il est positif (bon niveau de "curb appeal"), ou à anticiper avec prudence s'il est plutôt négligé (sans dramatiser, reste factuel et propose un angle honnête comme pour les autres points). RÉPARTITION AVEC LE MODE QUESTIONS & PIÈGES POUR ÉVITER LE DOUBLON : dans CE mode (Extérieurs), reste sur l'aspect VISUEL/IMPRESSION D'ENSEMBLE de la façade (esthétique, cohérence, premier regard) — NE traite PAS ici la performance thermique de l'enduit/l'isolation ni le chiffrage d'une façade à refaire comme poste de travaux (ça, c'est le rôle du mode Questions & Pièges, orienté technique/bâti). Si un matériau de qualité est mentionné (ex: enduit collé filet), tu peux le noter brièvement comme argument esthétique ici, mais le chiffrage/l'argument de durabilité technique détaillé reste la responsabilité de l'autre mode — ne duplique pas le même raisonnement en détail dans les deux.
PONDÉRATION SELON LA TAILLE ET LA QUALITÉ DE L'ABRI DE JARDIN : distingue bien les 3 niveaux déclarés, leur valeur diffère fortement — "Cabanon simple" (kit bois léger type grande distribution) : valeur modeste, autour de 300€-800€ selon la taille, argument mineur à ne mentionner que brièvement. "Cabanon qualitatif" (bois massif type chalet/autrichien) : nettement supérieur en qualité et en prix, généralement 1 500€ à 4 000€ selon la taille — un vrai argument de vente à mentionner explicitement, pas juste "il y a un cabanon". "Abri en dur" (maçonné/structure solide) : le plus valorisable, assimilable à une dépendance, argument fort. Module aussi selon la surface déclarée si disponible : un petit abri (moins de 6m²) reste un argument mineur même en qualité, tandis qu'un abri qualitatif ou en dur de taille conséquente (10m²+) mérite une mention explicite, potentiellement dans une question/réponse dédiée (ex: usage atelier, rangement volumineux). Si aucune surface n'est déclarée, reste neutre sur l'ampleur sans inventer une taille — la distinction de qualité (simple/qualitatif/dur) suffit déjà à donner le bon niveau d'argument même sans la taille.
ABRI BOIS/VÉLOS (auvent, appentis) : structure plus modeste et utilitaire qu'un cabanon, ne mérite pas le même niveau d'argument — reste un point pratique bref. SI le bien a aussi un poêle/chaudière à bois-granulés déclaré ailleurs dans le formulaire, fais le lien explicitement : un abri à bois dédié est un vrai argument pratique complémentaire (stockage au sec, séchage du bois), à mentionner ensemble plutôt que séparément (ex : "le poêle à bois est d'autant plus pratique qu'un abri dédié permet de stocker le bois au sec"). Sans poêle à bois, présente-le simplement comme un rangement pratique pour vélos/matériel extérieur, sans argument de chauffage à y associer.
Sois concret et direct, base-toi uniquement sur ce qui est visible sur les photos extérieures ou mentionné dans le texte — n'invente rien sur un jardin ou une façade que tu ne peux pas voir.` : ''}
