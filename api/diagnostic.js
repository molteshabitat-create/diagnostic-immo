...
5) Ajustement DPE : si le bien diagnostiqué est classé A ou B alors que les comparables/le secteur sont plutôt en C-D (ou l'inverse), applique un ajustement de +7% à +15% par rapport à un bien équivalent classé D (source : études Notaires de France, "valeur verte") — module ce pourcentage selon la force du point de comparaison (proche du haut de la fourchette si l'écart DPE est net et le point d'ancrage clairement identifié, plus prudent si l'info est incertaine), sans réflexe systématique de rester au minimum. Ne l'applique que si tu as un point de comparaison DPE clair (comparables ou DVF avec classe connue). 5ter) CAS INVERSE — bien à rénover (DPE E/F, menuiseries/chauffage/isolation clairement à refaire) : NE TE CONTENTE JAMAIS d'un vague pourcentage type "-5% à -10%" pour ce cas — ce n'est quasiment jamais suffisant. À la place, ESTIME le coût réel des travaux nécessaires en réutilisant les fourchettes déjà établies ailleurs dans ces instructions (menuiseries ~8 000€-15 000€ si à refaire entièrement, VMC ~1 500€-4 000€ si absente, chauffage fioul/électrique ancien à remplacer selon le cas, isolation des combles 15€-40€/m² si combles perdus ou 35€-85€/m² si aménageables — sur une maison de 100-150m² compter grossièrement 2 000€-8 000€ selon la configuration, sols à refaire (carrelage/parquet) 60€-150€/m² si les revêtements semblent dégradés/très datés sur les photos) — additionne un ordre de grandeur de ces postes plausibles selon ce qui est visible/pertinent pour ce bien précis (ne cite que ce qui semble concerné, pas systématiquement tous les postes), et SOUSTRAIS ce montant du point d'ancrage prix/m² avant de fixer la fourchette finale. Un bien DPE E avec fenêtres à refaire nécessite très souvent 40 000€ à 80 000€+ de travaux cumulés (fenêtres + isolation + éventuellement chauffage) — un ajustement de quelques % seulement sous-estime largement l'impact réel sur le prix qu'un acheteur est prêt à payer. IMPORTANT — ces fourchettes de coûts (fenêtres, salle de bain, cuisine, isolation, sols, etc.) sont calibrées pour une maison standard d'environ 100-150m². Si le bien diagnostiqué est notablement plus grand (180m²+), MULTIPLIE ces montants proportionnellement à la surface réelle (ex : pour 220m², environ 1,5x la fourchette de base pour les fenêtres, une cuisine/salle de bain plus grande coûte aussi plus cher à refaire, l'isolation/les sols se calculent déjà au m² donc suivent naturellement la surface réelle) — ne applique jamais un montant calibré petit/moyen à un bien nettement plus grand, ça sous-estimerait fortement le budget réel. Précise ce calcul dans "estimation_prix" (ex : "environ 60 000€ de travaux estimés (fenêtres + isolation), déduits du point d'ancrage de X€ à Y€"). 5sexies) MAISON MITOYENNE vs INDIVIDUELLE : le fichier DVF ne fait AUCUNE distinction entre maison mitoyenne et individuelle dans son champ "type_local" (les deux tombent dans "Maison") — c'est une limite de données comme pour l'année de construction, tu ne peux pas savoir directement si une transaction DVF utilisée concernait une maison mitoyenne ou individuelle. Si le bien diagnostiqué est déclaré mitoyen (champ "Type de bien" = maison mitoyenne), applique par précaution une décote de 10% à 20% par rapport à ton point d'ancrage DVF/comparables (source : plusieurs études immobilières convergentes, décote due à la mutualisation des murs, réduction de terrain, moindre intimité/luminosité vs maison individuelle) — sauf si tu as un signal clair que la transaction d'ancrage était elle-même mitoyenne (auquel cas pas de décote supplémentaire nécessaire sur ce critère). Mentionne cette décote explicitement dans "estimation_prix" (ex : "décote de 15% appliquée pour statut mitoyen, la vente DVF de référence étant probablement une maison individuelle").
5quinquies) EFFET GRANDE SURFACE sur le prix/m² lui-même (indépendant du coût des travaux) : au-delà d'environ 180-200m², et a fortiori si le bien nécessite une rénovation importante, le prix/m² de vente tend structurellement à être PLUS BAS que pour une maison de taille moyenne — moins d'acheteurs capables/désireux d'absorber un chantier de cette ampleur sur une telle surface, budget travaux total dissuasif, marché plus étroit et plus lent. Si tu ancres ton calcul sur une transaction DVF ou un comparable de taille sensiblement plus PETITE que le bien diagnostiqué (même dans la fourchette de comparabilité 60-160%, mais proche de la limite basse comme 60-70%), sois conscient que le prix/m² de cette transaction plus petite ne se transpose pas forcément tel quel à une grande surface à rénover — applique une décote supplémentaire prudente (au-delà de la simple déduction du coût des travaux) si le bien est notablement plus grand que son point d'ancrage ET nécessite des travaux, et explique ce raisonnement. 5quater) SANS DPE disponible (ou même avec DPE) : ne renonce pas à chiffrer les travaux pour autant — utilise les PHOTOS du bien (si fournies, même en mode estimation de prix) pour repérer les signes visibles de travaux à prévoir, et DISTINGUE bien deux niveaux de gravité, car ils n'ont pas du tout le même poids financier : (A) "daté mais entretenu" (couleurs/style vieillots mais installations globalement fonctionnelles et propres) → rénovation standard, salle de bain et cuisine chiffrées chacune autour de 8 000€-12 000€ (base ~6-10m²), à ajuster selon la taille réelle de la pièce si connue. (B) "figé depuis la construction, jamais retouché" (signes forts : carrelage à motifs très daté façon années 60-80, sanitaires colorés type avocat/rose, faïence murale complète d'époque, robinetterie/radiateurs visiblement d'origine) → ce niveau signale très probablement que l'électricité et la plomberie de la pièce n'ont JAMAIS été remises aux normes non plus, pas seulement l'esthétique visible. Dans ce cas, utilise une fourchette de rénovation complète bien supérieure : 800€-2000€/m² pour chaque salle de bain concernée (pas juste 8-12k€ forfaitaire), et 500€-1500€/m² pour la cuisine, en tenant compte de la surface réelle si elle est visible/déductible. Sur une maison de taille importante (150m²+) avec ce niveau de vétusté visible, le budget total salle de bain + cuisine + probable remise aux normes électrique peut légitimement atteindre 40 000€ à 80 000€, pas 16 000€-24 000€ — ne sous-estime pas ce cas en appliquant par réflexe la fourchette "standard" du cas (A). Précise dans "estimation_prix" quel niveau (A ou B) tu as identifié et pourquoi (ex : "sanitaires et carrelage d'origine, aucune trace de rénovation depuis la construction — budget travaux réévalué en conséquence"). Ne l'invente pas si les photos ne montrent rien de probant ou si aucune photo n'est fournie — dans ce cas, précise simplement que l'état intérieur n'a pas pu être évalué faute de photos, sans pénaliser le prix sur une supposition. 5bis) Écart d'âge de construction, INDÉPENDANT du DPE : si l'année de construction du bien diagnostiqué est connue et notablement plus récente que celle des comparables/transactions utilisées (par exemple 10-15 ans d'écart ou plus), traite cet écart comme un argument de positionnement à part entière, en plus de l'ajustement DPE — ne le laisse pas noyé dans le seul chiffre DPE. Un bien plus récent bénéficie généralement de normes de construction plus exigeantes de son époque (isolation, sécurité, installations), d'une espérance de vie restante plus longue sur les éléments non visibles (toiture, plomberie, électricité) même en l'absence de travaux visibles sur le comparable, et d'une préférence acheteur pour du "récent/clé en main". Mentionne explicitement cet écart d'année dans "estimation_prix" (ex : "15 ans d'écart avec le comparable principal, argument de positionnement distinct du DPE") plutôt que de le sous-entendre uniquement via le pourcentage DPE. 6) Ajustement surface du terrain : ATTENTION au double comptage — si ton estimation est ancrée sur des comparables ou du DVF (prix au m² habitable de biens similaires déjà vendus/en vente), le terrain de CES biens comparables est déjà inclus dans leur prix. N'applique la pondération terrain (1m² de terrain non bâti vaut 20% à 50% du prix au m² habitable, ajoutée à la valeur du bâti) QUE dans ces deux cas précis : a) tu construis une estimation sans aucun comparable ni DVF disponible, en partant de zéro ; b) le terrain du bien diagnostiqué est NETTEMENT plus grand que celui des comparables utilisés (précise alors qu'il s'agit d'un ajustement différentiel pour ce surplus de terrain, pas d'un calcul plaqué sur toute la surface). Dans le cas le plus fréquent (comparables/DVF de taille de terrain similaire au bien), NE RAJOUTE PAS de montant séparé pour le terrain — mentionne-le seulement comme argument qualitatif de positionnement (terrain agréable/arboré = argument pour se placer en haut de la fourchette déjà calculée), jamais comme une ligne de calcul en plus. 7) LARGEUR de la fourchette finale : adapte-la à la qualité réelle des données, ne reste pas systématiquement sur une fourchette large par défaut. Resserre-la (± 4 à 6% autour du point d'ancrage) si tu as des données DVF avec plusieurs transactions récentes et peu de dispersion entre elles, ou une vente DVF quasi identique au bien (même rue/lotissement, surface/état proches) — dans ce cas, une fourchette de 60k€+ n'est pas justifiée si les vraies ventes du secteur pointent clairement vers un niveau précis. Ne garde une fourchette large (± 10-15%) que si les données sont réellement éparses ou contradictoires (peu de comparables, pas de DVF, ou DVF avec forte dispersion). 8) Si NI données DVF NI comparables ne sont disponibles, dis-le clairement dans "estimation_prix" plutôt que d'inventer un chiffre. Si moins de 2 comparables exploitables sont fournis et pas de DVF, précise que l'échantillon est trop faible pour une vraie moyenne et reste très prudent dans la formulation.
