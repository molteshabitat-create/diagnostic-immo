...
5) APRÈS AVOIR DÉTERMINÉ LE POINT D'ANCRAGE (prix/m² × surface), applique SYSTÉMATIQUEMENT cette checklist dans l'ordre, un point après l'autre — ne saute AUCUN point qui s'applique à ce bien, même si le texte final doit rester concis (résume plusieurs points en une phrase si besoin, mais n'en ignore aucun dans ton raisonnement) :
  ...
  i) SI LE BIEN EST UN APPARTEMENT (pas une maison) — étage, ascenseur, vue (source : études SeLoger/MeilleursAgents) : rez-de-chaussée → décote de 8% à 10% vs un étage courant. Dernier étage AVEC ascenseur → prime de 15% à 19% vs rez-de-chaussée (jusqu'à 25% pour une ancienne chambre de bonne au dernier étage). RÈGLE D'INFÉRENCE PAR DÉFAUT SUR L'ASCENSEUR : si l'annonce ou les caractéristiques complémentaires ne mentionnent PAS explicitement un ascenseur, ET que l'étage est le 3e ou plus, considère par défaut qu'il n'y a PROBABLEMENT PAS d'ascenseur — une agence immobilière ne manque quasiment jamais de mentionner un ascenseur quand il y en a un (c'est un argument de vente trop fort pour l'omettre), donc son absence de mention est un signal fiable de son absence réelle. Applique alors la décote SANS ascenseur : 3e étage → décote d'environ 10% ; 4e étage et plus → décote de 15% à 20% (source : SeLoger, "un logement peut perdre jusqu'à 20% de sa valeur à partir du 4e étage sans ascenseur"). Ne suppose l'ascenseur présent que si explicitement confirmé dans le texte. Pour un immeuble SANS ascenseur confirmé, les étages 1-2 restent recherchés (pas de décote), la décote ne s'active qu'à partir du 3e étage. N'applique ces ajustements que si l'étage lui-même est connu — si même l'étage est inconnu, ne devine rien. Une vue dégagée/sur monument ou une exposition sud/traversante déclarée peuvent aussi justifier une prime qualitative modérée, à mentionner sans lui donner un pourcentage chiffré précis (contrairement à l'étage/ascenseur qui sont bien sourcés).
  j) COHÉRENCE ENTRE LE NIVEAU DE L'ANCRAGE ET LES FINITIONS RÉELLES DU BIEN : une transaction DVF ou un comparable au prix/m² très élevé (haut de la fourchette locale) reflète très probablement un bien avec des finitions haut de gamme — ce n'est PAS le niveau de prix "normal" pour n'importe quel bien de cette taille. UTILISE L'ANALYSE VISUELLE DU BIEN fournie plus haut (niveau_finition_cuisine, niveau_finition_salle_de_bain) pour trancher : "premium" sur les deux → l'ancrage haut est justifié. "standard" ou "a_renover" → le bien ne correspond probablement PAS au niveau de l'ancrage le plus haut. DEUX CAS SELON LES DONNÉES DISPONIBLES : 1) S'il existe PLUSIEURS transactions de taille comparable avec des prix/m² différents, choisis la transaction au prix/m² intermédiaire (pas la plus haute) comme nouvel ancrage. 2) S'il n'existe qu'UNE SEULE transaction de taille comparable (cas fréquent sur un échantillon réduit) et que ses finitions sont probablement premium (prix/m² nettement au-dessus du reste du marché local), tu n'as pas d'alternative vers laquelle basculer — dans ce cas, APPLIQUE UNE DÉCOTE DIRECTE ET CHIFFRÉE de 10% à 20% sur le point d'ancrage pour refléter l'écart de gamme (standard vs premium), plutôt que de laisser l'ancrage haut inchangé faute d'alternative. Précise ce calcul dans "estimation_prix" (ex : "ancrage à X€/m² probablement premium (seule transaction de taille comparable), décote de 15% appliquée pour des finitions standards, soit Y€/m² retenu"). ATTENTION à une confusion fréquente : "bon état/propre/entretenu" N'EST PAS la même chose que "haut de gamme" — une cuisine standard impeccablement entretenue reste "standard", elle ne justifie PAS un positionnement haut de fourchette juste parce qu'elle est en bon état. C'EST UN POINT OBLIGATOIRE de la checklist, ne le saute pas, et il DOIT se traduire par un chiffre concret dans le calcul, jamais rester une simple remarque textuelle sans impact sur la fourchette finale.
...
Sois STRICT sur "premium" : ne l'utilise que si tu vois clairement des matériaux/marques haut de gamme (pierre naturelle massive, marques identifiables haut de gamme, agencement sur-mesure fait par un architecte/cuisiniste, robinetterie design) ET un état impeccable. EXEMPLES DE CE QUI N'EST PAS PREMIUM même si propre et en bon état : cuisine blanche basique avec plan de travail stratifié/mélaminé imitation bois (type IKEA, Lapeyre, Brico Dépôt, grande distribution), faïence murale simple, sanitaires standards de salle de bain (blanc classique, sans marque distinctive) — tout ça reste "standard", même impeccablement propre et récent. "Propre et en bon état" n'est PAS un synonyme de "premium" : évalue les MATÉRIAUX eux-mêmes (leur nature, leur coût probable), pas juste leur état d'entretien. En cas de doute entre standard et premium, choisis "standard" — le risque de sous-estimer une belle cuisine est bien moindre que celui de surestimer un bien à cause d'une cuisine grand public prise pour du haut de gamme.`
      }
    ];
